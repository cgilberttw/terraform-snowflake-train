...
echo "Account: $SNOWFLAKE_ACCOUNT_LOCATOR | User: $SNOWFLAKE_USER | Role: $SNOWFLAKE_ROLE"
echo ""

# Test 1: Basic connection and current context
echo "Current session info"
snowsql -a $SNOWFLAKE_ACCOUNT_LOCATOR \
       -u $SNOWFLAKE_USER \
       -r $SNOWFLAKE_ROLE \
       --private-key-path $SNOWFLAKE_PRIVATE_KEY_LOCAL_PATH \
